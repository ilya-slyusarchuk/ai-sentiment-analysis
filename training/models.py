...
if __name__ == '__main__':
  ...
  video_data = sample['video_frames'].unsqueeze(0)
  audio_data = sample['audio_features'].unsqueeze(0)
  ...
  with torch.inference_mode():
    outputs = model(text_inputs, video_data, audio_data)
    emotion_probabilities = torch.softmax(outputs['emotions'], dim=1)[0]
    sentiment_probabilities = torch.softmax(outputs['sentiments'], dim=1)[0]
